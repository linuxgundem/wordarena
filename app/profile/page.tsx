...
import { User, Medal, Trophy, Activity, ArrowLeft, Settings, Award, Loader2 } from 'lucide-react'
...
// Stats ve Rozetleri şimdilik sabit (dummy) tutabiliriz, veritabanına eklenince güncellenir
const dummyStats = {
  gamesPlayed: 0,
  wins: 0,
  winRate: '0%',
...
const dummyBadges = [
  { id: 1, name: 'İlk Galibiyet', icon: '🏆', color: 'bg-yellow-500/20 text-yellow-500 border-yellow-500/50', locked: true },
  { id: 2, name: 'Kelime Cambazı', icon: '✨', color: 'bg-purple-500/20 text-purple-500 border-purple-500/50', locked: true },
  { id: 3, name: '100. Oyun', icon: '🎯', color: 'bg-blue-500/20 text-blue-500 border-blue-500/50', locked: true },
  { id: 4, name: 'Kusursuz Tur', icon: '🔥', color: 'bg-red-500/20 text-red-500 border-red-500/50', locked: true },
]
...
export default function ProfilePage() {
  ...
  // Sonraki seviye için gereken XP hesaplaması (basit bir formül: level * 1000)
  const nextLevelXp = profile.level * 1000
  const xpPercentage = (profile.xp / nextLevelXp) * 100
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start gap-8 relative z-10">
            ...
            <div className="relative">
              <div className="w-32 h-32 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center shadow-[0_0_40px_rgba(59,130,246,0.3)]">
                <span className="text-5xl font-black text-white">{profile.username.substring(0,2).toUpperCase()}</span>
              </div>
            ...
            <div className="flex-1 w-full mt-4 md:mt-2 text-center md:text-left">
              ...
              <p className="text-neutral-400 mb-6 flex items-center justify-center md:justify-start">
                <Award className="w-4 h-4 mr-1 text-blue-400" /> Çaylak Oyuncu
              </p>

              <div>
                <div className="flex justify-between text-sm font-medium mb-2">
                  <span className="text-neutral-300">Seviye İlerlemesi</span>
                  <span className="text-blue-400">{profile.xp} / {nextLevelXp} XP</span>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-1 space-y-4">
            ...
            <div className="bg-neutral-900/40 border border-neutral-800 rounded-2xl p-5 flex items-center justify-between">
              ...
              <span className="text-2xl font-bold">{dummyStats.gamesPlayed}</span>
            </div>
            <div className="bg-neutral-900/40 border border-neutral-800 rounded-2xl p-5 flex items-center justify-between">
              <span className="text-neutral-400">Kazanma Sayısı</span>
              <span className="text-2xl font-bold text-green-400">{dummyStats.wins}</span>
            </div>
            <div className="bg-neutral-900/40 border border-neutral-800 rounded-2xl p-5 flex items-center justify-between">
              <span className="text-neutral-400">Kazanma Oranı</span>
              <span className="text-2xl font-bold text-yellow-400">{dummyStats.winRate}</span>
            </div>
            <div className="bg-neutral-900/40 border border-neutral-800 rounded-2xl p-5 flex items-center justify-between">
              <span className="text-neutral-400">Toplam Puan</span>
              <span className="text-2xl font-bold text-purple-400">{dummyStats.totalPoints}</span>
            </div>
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {dummyBadges.map((badge, index) => (
                <div key={badge.id}>
                  {badge.locked ? (
                    <div className="h-full border border-neutral-800 border-dashed rounded-2xl p-4 flex flex-col items-center justify-center text-center bg-neutral-900/20">
                      <div className="w-8 h-8 rounded-full bg-neutral-800 mb-2 flex items-center justify-center text-neutral-600">?</div>
                      <span className="text-xs font-medium text-neutral-600">Kilitli: {badge.name}</span>
                    </div>
                  ) : (
                    <motion.div
                      ...
                      className={`h-full border rounded-2xl p-4 flex flex-col items-center justify-center text-center backdrop-blur-sm ${badge.color}`}
                    >
